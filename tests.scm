...
(define fc-barcelona 'real-madrid-sucks)
; expect fc-barcelona
fc-barcelona
;expect real-madrid-sucks

(define uefa (list 'Madrid 'Atletico 'Monaco 'Juventus))
;expect uefa
uefa
;expect (madrid atletico monaco juventus)

(quote (what I learned in boating school was...))
;expect (what i learned in boating school was...)

(define z (append (quote (mary had)) (quote (a little lamb))))
;expect z
z
;expect (mary had a little lamb)

(begin (define w 2) (define x 3) (define y 1) (define z 9) (define a (list w x y z)) (quote (we have)))
;expect (we have)
a
;expect (2 3 1 9)

(begin (quote (This should not print)) (begin (quote cute) (> 23 19) (begin (quote (so much recursion)) (define tomorrow 'today) (define yesterday 'tonight))))
;expect yesterday
tomorrow
;expect today
yesterday
...
(define pythag (lambda (x y) (+ (* x x) (* y y))))
;expect pythag
(pythag 3 4)
;expect 25
(define z (lambda (x y) (sqrt (pythag x y))))
;expect z
(z 3 4)
;expect 5.0
...
(begin (define w 1) (define x 2) (define y nil) (define z 10))
(define (function w x y z)
  (if (or w z)
    (and w z)
    (if (and x y)
        (or x y)
...
(or #f (define function (lambda (y) (* y y))) (and (define x 90000) 4) pythag)
;expect function
...
(cond
    ((null?(pythag 1 2)) 'nah)
    ((null? 2) 'wut)
    ((and 3 4 5 6 7 (pythag 3 4) #f) 'no)
    (else '(there we go)))
;expect (there we go)

(define (kind-of-triangle x y z)
  (cond
      ((and (= x y) (= y z) (= z x))  'equalateral)
      ((or (= x y) (= y z) (= z x)) 'isosceles)
      (else 'scalene)))

(kind-of-triangle 2 2 3)
;expect isosceles

(define primary (mu (a) (* a b)))
primary
(define next (lambda (a b) (primary (* a a))))
next
