...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if (formals is nil and vals is not nil) or (vals is nil and formals is not nil):
            raise SchemeError('Length of formals is not the same as the length of values.')
        # END PROBLEM 11
        return child
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formals.append(bindings.first.first)
        vals.append(scheme_eval(bindings.first.second.first, env))
        bindings = bindings.second

    def make_pair(lst):
        if len(lst) == 1:
            return Pair(lst[0], nil)
        return Pair(lst[0], make_pair(lst[1:]))

    pairform = make_pair(formals)
    check_formals(pairform)
    return env.make_child_frame(pairform, make_pair(vals))
    # END PROBLEM 15
